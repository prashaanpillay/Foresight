...
class RetrieveDatasetCommand(Command):
    ...
    def __init__(self):
        ...
        self.acquisition_config = JsonParser.decode(
            self.assetLoader.get_config('acquisition.json'))
    ...
    def read_in_pickle_files(self):
        ...
        training_dataframe = pandas.read_pickle(self.raw_directory +
                                                self.dataset["training_set_input_directory"])

        for row in training_dataframe.iterrows():
            candidate_model = CandidateModel()
            candidate_model.update(row[1]['survey_id'], self.get_path(row[1]['anchor_image_path']), row[1]['transform'],
                                   self.get_path(row[1]['ref_image_path']), self.get_path(row[1]['target_image_path']))
            candidate_training_models.append(candidate_model)
            candidate_model.write_out(self.dataset_model.training_directory)

        validation_dataframe = pandas.read_pickle(self.raw_directory +
                                                  self.dataset["validation_set_input_directory"])
    ...
    def setup_output_directory(self):
        training_directory = os.path.abspath(
            self.build["training_set_output_directory"])
        validation_directory = os.path.abspath(
            self.build["validation_set_output_directory"])
        ...
        self.logger.log("Training set written to: " +
                        str(self.dataset_model.training_directory))
        self.logger.log("Validation set written to: " +
                        str(self.dataset_model.validation_directory))
